...
public class Wysa_TC2_9 {
	static void tc29(DesiredCapabilities dc, AndroidDriver<AndroidElement> driver, WebDriverWait wait) throws IOException {
...
//(MobileElement) //driver.findElementByXPath("//android.widget.RelativeLayout[@content-desc=\"Urge //Surfing\"]");
		...
		el5.sendKeys("How can i create self awarness?");
		driver.manage().timeouts().implicitlyWait(50, TimeUnit.SECONDS);
		...
		el9.sendKeys("any suggestions to create self-awareness?");
		driver.manage().timeouts().implicitlyWait(50, TimeUnit.SECONDS);
		...
		FileUtils.copyFile(scrFile2, new File("2.1." + ".jpg"));
		System.out.println("Screenshot taken and saved to folder.");
		String video = driver.stopRecordingScreen();

        byte[] decode = Base64.getDecoder().decode(video);

        FileUtils.writeByteArrayToFile(new File("2.9.mp4"), decode);
