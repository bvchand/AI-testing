...
public class Wysa_TC2_12 {
	static void tc212(DesiredCapabilities dc, AndroidDriver<AndroidElement> driver, WebDriverWait wait) throws IOException {
...
//(MobileElement) //driver.findElementByXPath("//android.widget.RelativeLayout[@content-desc=\"Urge //Surfing\"]");
		...
		FileUtils.copyFile(scrFile2, new File("2.12" + ".jpg"));
		System.out.println("Screenshot taken and saved to folder.");
		String video = driver.stopRecordingScreen();

        byte[] decode = Base64.getDecoder().decode(video);

        FileUtils.writeByteArrayToFile(new File("2.12.mp4"), decode);
